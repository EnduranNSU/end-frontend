  ...
  <q-page class="q-pa-md main-page">
    ...
    <section class="section">
      <div class="cards-grid">
        ...
        <q-card v-else-if="items.length" v-for="it in items" :key="it.id" class="card new-workout" @click="openPerformed(it.id)">
          <div class="card-body">
...
.section-title { text-align: center; margin: 12px 0 }
.cards-grid { display:grid; grid-template-columns: repeat(auto-fill, minmax(160px,1fr)); gap:12px; padding:8px 12px 0 }
.card { height:96px; border-radius:14px }
.card .card-body { display:flex; align-items:center; justify-content:center; position:relative }
.card .card-title { font-weight:600 }
.card .card-dots { position:absolute; right:6px; top:6px; color:#bdbdbd }
</style>
